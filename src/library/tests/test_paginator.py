""""Test paginator modul."""


from library.paginator import Paginator


def test_empty_list():
    """Test whether an empty list is handled correctly."""
    items = []
    page = Paginator(items, page=0)
    assert page.page == 0
...
def test_one_page():
    ...
    items = range(10)
    page = Paginator(items, page=0, items_per_page=10)
    assert page.page == 1
...
def test_many_pages():
    ...
    items = range(100)
    page = Paginator(items, page=0, items_per_page=15)
    assert page.page == 1
    assert page.first_item == 1
